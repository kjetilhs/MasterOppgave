...
The prinsiple of \arcfull{rtk-gps} is the use of a base station and a rover, where the distance between them are calculated in real time while the rover is in motion. The distance between the base station and the rover is refereed to a base line. The length of the base line effect the accuracy of the \gls{rtk-gps} solution, however with a base line less then $1-2 km$ the solution can be considered with centimetre level accuracy.

The base station sends raw \gls{gps} data over a radio link to the rover. An important feature for \gls{rtk-gps} is the ability to estimate the integer ambiguity while the rover is in motion. A well used method was purposed in the article \citep{teunissen1994new} which decorrelate the integer ambiguities such that a efficient computation of the least square estimate can be performed. The search method is further explained in \citep{teunissen1995least}, where the estimates from the different steps in the method is referred to fixed and float. When the solution is categorised as fixed the accuracy of the solution is considered on centimetre level 
