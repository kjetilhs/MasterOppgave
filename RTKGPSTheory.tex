...
The base station sends raw \gls{gps} data over a radio link to the rover. An important feature for \gls{rtk-gps} is the ability to estimate the integer ambiguity while the rover is in motion. A well used method was purposed in the article \citep{teunissen1994new} which decorrelate the integer ambiguities such that a efficient computation of the least square estimate can be performed. The type of solution can be categorised as either a fixed or float solution. NEED REF When the solution is categorised as fixed the accuracy of the solution is considered on centimetre level
