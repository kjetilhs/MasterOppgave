...
\section{JSBsim}
JSBsim\citep{jsbsim2016} is an open-source flight dynamic model that simulate a physical model of a aircraft, which is affected by external forces and moments. The external forces can be various control mechanism or environmental forces. JSBsim is able to receive control input from external program like ardupilot, which allows for testing of various control design in an external program.

JSBsim is configured through a XML configuration file 